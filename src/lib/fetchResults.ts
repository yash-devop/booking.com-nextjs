...
export default async function fetchResults(searchParams: SearchParams){
    const username = process.env.OXYLABS_USERNAME;
    const password = process.env.OXYLABS_PASSWORD;
    const url = new URL(searchParams.url);

    // we are now looping through the keys from the object of the searchparams;
    ...
    // setting up the Oxylab web scrapper so that
    // whatever the data is in Booking.com , we will get that into our NEXTJS application
    ...
    next: {
      revalidate: 60 * 60, // cache for 1 hour , every 1 hr refresh or cache basically.
    },
    headers: {
